...
const ProjectCard = ({
  ...
  return (
    <article className=" aspect-[10/11] shrink-0 w-[85vw] md:w-[600px] group relative rounded-[2rem] overflow-hidden sm:aspect-[16/10] snap-center">
      <img
        alt={imgAlt}
        className="absolute inset-0 w-full h-full object-center object-cover blur-sm sm:blur-none transition-transform duration-1000 group-hover:scale-110"
        src={imgSrc}
      />
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent group-hover:z-2" />
      <div className="absolute inset-x-6 bottom-6 flex justify-between items-end">
        <div className="flex justify-between items-center bg-white/4 p-6 rounded-2xl border border-white/10 w-full  backdrop-blur-3xl">
          <div>
      ...
      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center z-10 backdrop-blur-xl">
        <a
          href={link}
          target="_blank"
